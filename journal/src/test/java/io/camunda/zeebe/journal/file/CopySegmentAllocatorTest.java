...
final class CopySegmentAllocatorTest {
  @Test
  void shouldUseFallbackWhenFallocateNotSupported(final @TempDir Path tmpDir) throws IOException {
    ...
    final var allocator = CopySegmentAllocator.createAllocator(tmpDir, size, linuxFs);
    ...
    Assertions.assertThat(linuxFs.isCopyFileRangeEnabled())
        .as("has disabled copyFileRange")
        .isFalse();
  }
  ...
  void shouldPreallocateFile(final @TempDir Path tmpDir) throws IOException {
    ...
    final var segmentFile = tmpDir.resolve("file");
    final var size = 1024 * 1024;
    final var allocator = CopySegmentAllocator.createAllocator(tmpDir, size);
