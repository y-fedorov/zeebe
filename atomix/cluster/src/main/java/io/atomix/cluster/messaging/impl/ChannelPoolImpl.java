...
class ChannelPoolImpl implements ChannelPool {
  private static final Logger LOGGER = LoggerFactory.getLogger(ChannelPoolImpl.class);
  private static final Histogram OFFSET_DISTRIBUTION =
      Histogram.build()
          .namespace("atomix")
          .name("channel_pool_distribution")
          .help("Distribution of request count per connection")
  ...
  ChannelPoolImpl(final Function<Address, CompletableFuture<Channel>> factory, final int size) {
   ...
   */
  @Override
  public CompletableFuture<Channel> getChannel(final Address address, final String messageType) {
    final List<CompletableFuture<Channel>> channelPool = getChannelPool(address);
    final int offset = getChannelOffset(messageType);

    OFFSET_DISTRIBUTION.observe(offset);
