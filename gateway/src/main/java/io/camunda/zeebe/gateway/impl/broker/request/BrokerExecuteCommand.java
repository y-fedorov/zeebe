...
public abstract class BrokerExecuteCommand<T> extends BrokerRequest<T> {
  ...
  public BrokerExecuteCommand(final ValueType valueType, final Intent intent) {
    this(valueType, intent, Span.current().getSpanContext());
  }

  public BrokerExecuteCommand(
      final ValueType valueType, final Intent intent, final SpanContext spanContext) {
